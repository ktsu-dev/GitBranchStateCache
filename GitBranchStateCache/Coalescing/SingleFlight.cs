...
public sealed class SingleFlight : ISingleFlight
{
	...
	private void Retire(string key, Entry entry)
	{
		...
		// Only remove the entry this ticket owns, so a newer operation for the same key is untouched.
	...
	private sealed class Ticket(SingleFlight owner, string key, Entry entry, bool isLeader) : IWorkTicket
	{
		...
		public async Task<bool> WaitForLeaderAsync(TimeSpan timeout, CancellationToken cancellationToken)
		{
			...
			{
				// The leader is stalled. Reported as a failure so this follower proceeds on its own.
				return false;
			}
		}
		...
		public void Complete(bool succeeded)
		{
			if (!isLeader)
			{
				throw new InvalidOperationException("Only the leader reports the outcome.");
			}

			entry.Completion.TrySetResult(succeeded);
		...
		public void Dispose()
		{
			...
			// leaving them waiting for work that is no longer happening.
