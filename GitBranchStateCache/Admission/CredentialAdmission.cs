...
{
	/// <summary>
	/// How many entries may accumulate before expired ones are swept.
	/// </summary>
	/// <remarks>
	...
	{
		// An anonymous caller is never admitted. A public repository would still be readable without a
		// credential, but this service must not be the thing that decides that, so it declines to
		// answer rather than proving nothing and serving anyway.
	...
	public void Admit(string upstream, string repositoryPath, string? authorization)
	{
		...
		_admitted[Key(upstream, repositoryPath, authorization)] =
			timeProvider.GetUtcNow() + options.Value.AdmissionTtl;
	...
	/// </summary>
	/// <remarks>
	/// The three parts are separated by a character that cannot appear in an upstream key or a
	/// repository path, so no two different triples can produce the same input. Without that, an
	/// upstream and a repository could be re-split to match a different pair.
	/// </remarks>
	private string Key(string upstream, string repositoryPath, string authorization)
	{
		...
		// alternative, an instance per call, allocates on a path taken by every request.
		lock (_hashGate)
		{
